# Generate committed cross-validation vectors for the Compact Hilbert curve.
#
# This script builds the HilbertCurveCompact Zig shared library (the reference
# implementation), calls its C ABI via ccall, and writes test/compact_vectors.txt
# with the reference Hilbert indices. It also cross-checks the Julia Compact
...
const OUT_FILE = normpath(joinpath(@__DIR__, "..", "test", "compact_vectors.txt"))
const SEED = 20260706
...
function main()
    ...
    open(OUT_FILE, "w") do io
        println(io, "# Compact cross-validation vectors.")
        ...
        for (m, mode, npoints, sels) in config_matrix()
            ...
            for (gray, path, gi, pi) in sels
                g = Compact(mi; gray = gray, path = path, gray_index = gi, path_index = pi)
                T = index_type(g)
                B = axis_type(g)
    ...
    if total_mismatch == 0
        println("\nAll Julia Compact indices match the Zig reference (0 mismatches).")
    else
