...
"""
    CompactHamilton{T,B}(m::AbstractVector{<:Integer})
    CompactHamilton(T, m::AbstractVector{<:Integer})
    CompactHamilton(m::AbstractVector{<:Integer})

Deprecated: `CompactHamilton` is the pre-0.7 `Compact` algorithm (Hamilton &
Rau-Chaplin conventions), kept for one release cycle for users who persisted
their anisotropic Hilbert indices. New code should use [`Compact`](@ref), which
produces identical indices on uniform grids but a different (still
lattice-continuous) curve on anisotropic grids. `CompactHamilton` will be
...
# Example
```julia
c = CompactHamilton{UInt64, UInt32}([3, 2, 4])  # Dimension [2^3, 2^2, 2^4]
...
"""
struct CompactHamilton{T, B} <: HilbertAlgorithm{T}
...
function CompactHamilton{T,B}(m::AbstractVector{<:Integer}) where {T<:Unsigned, B<:Unsigned}
    # Validate inputs
    n = length(m)
    ...
    if mmax == 0
        return CompactHamilton{T,B}(copy(m), 0, 0, Int[], zeros(Int, n, 0), zeros(Int, n, 0))
    end
    ...
    CompactHamilton{T,B}(copy(m), mmax, total_bits, k_level, axes_level, pos_level)
end


function CompactHamilton(m::AbstractVector{<:Integer})
    ...
    CompactHamilton{T,B}(m)
end


function CompactHamilton(T, m::AbstractVector{<:Integer})
    U = unsigned(T)
    B = large_enough_unsigned(maximum(m))   # coord type from max bits per axis
    CompactHamilton{U,B}(m)
end


axis_type(::CompactHamilton{T,B}) where {T,B} = B
...
"""
    encode_hilbert_zero(c::CompactHamilton, X)

Encode a point X (0-based coordinates) to a Hilbert index (0-based).
"""
function encode_hilbert_zero(c::CompactHamilton{T,B}, X::AbstractVector)::T where {T,B}
...
"""
    decode_hilbert_zero!(c::CompactHamilton, X, h)

Decode a Hilbert index h (0-based) to point X (0-based coordinates).
"""
function decode_hilbert_zero!(c::CompactHamilton{T,B}, X::AbstractVector, h::T) where {T<:Integer,B}
...
function decode_hilbert_zero!(c::CompactHamilton{T,B}, X::AbstractVector, h::Integer) where {T,B}
