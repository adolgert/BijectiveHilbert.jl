# Glued-seam Hilbert curve for anisotropic grids (different resolution per axis).
#
...
# state embedding is needed (contrast src/compact.jl, which sorts ascending and
# uses `embed_state`). The affine state uses the paper's rotate-by-`d`
# convention (`glued_affine_apply`), NOT Hamilton's `d+1` convention.
...
"""
    GluedSeam{T,B}(m::AbstractVector{<:Integer}; kwargs...)
    GluedSeam(T, m::AbstractVector{<:Integer}; kwargs...)
    GluedSeam(m::AbstractVector{<:Integer}; kwargs...)
...
# Example
```julia
g = GluedSeam([3, 2, 4])                                  # default BRGC curve
...
g2 = GluedSeam([4, 3, 2, 2]; gray=:random, path=:catalog, gray_index=2, path_index=7)
```

# Relationship to [`Compact`](@ref)
`GluedSeam` makes the same anisotropic, lattice-continuous promise as `Compact`
but adds a choice of curve family (`gray`/`path`) and uses a different axis-order
convention (axes sorted by descending bit count, rotate-by-`d` affine state), so
its anisotropic Hilbert indices generally differ from those of `Compact`.

This code follows the paper "Gluing the Seam of a Hilbert Curve" (Andrew Dolgert,
2026) and ports the affine transducer from the HilbertCurveCompact library.
"""
struct GluedSeam{T, B} <: HilbertAlgorithm{T}
...
function GluedSeam{T, B}(m::AbstractVector{<:Integer};
                         gray::Symbol = :brgc,
    ...
    if max_m == 0
        return GluedSeam{T, B}(n, fill(0, n), collect(1:n), Int[], 0, 0,
    ...
    GluedSeam{T, B}(n, m_bits, axis_perm, k_levels, m_sum, max_m, tables,
                    gray, path, Int(gray_index), Int(path_index))
end


function GluedSeam(m::AbstractVector{<:Integer}; kwargs...)
    ...
    GluedSeam{T, B}(m; kwargs...)
end


function GluedSeam(T, m::AbstractVector{<:Integer}; kwargs...)
    U = unsigned(T)
    B = large_enough_unsigned(maximum(m))   # coord type from max bits per axis
    GluedSeam{U, B}(m; kwargs...)
end


axis_type(::GluedSeam{T, B}) where {T, B} = B


function Base.show(io::IO, g::GluedSeam{T, B}) where {T, B}
    m = zeros(Int, g.n)
    for j in 1:g.n
        m[g.axis_perm[j]] = g.m_bits[j]
    end
    print(io, "GluedSeam{$T,$B}(", m, "; gray=:", g.gray, ", path=:", g.path,
...
"""
    decode_hilbert_zero!(g::GluedSeam, X, h)
...
"""
function decode_hilbert_zero!(g::GluedSeam{T, B}, X::AbstractVector, h::T) where {T <: Integer, B}
...
function decode_hilbert_zero!(g::GluedSeam{T, B}, X::AbstractVector, h::Integer) where {T, B}
