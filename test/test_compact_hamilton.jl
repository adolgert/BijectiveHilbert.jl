using TestItemRunner


@testitem "CompactHamilton constructor validation" begin
    using BijectiveHilbert: CompactHamilton

    # Valid construction
    c = CompactHamilton{UInt64, UInt32}([3, 2, 4])
    @test c.mmax == 4
    @test c.total_bits == 9
    @test length(c.m) == 3

    # Edge case: uniform dimensions
    c2 = CompactHamilton{UInt64, UInt32}([3, 3, 3])
    @test c2.mmax == 3
    @test c2.total_bits == 9

    # Edge case: single dimension
    c3 = CompactHamilton{UInt64, UInt32}([5])
    @test c3.mmax == 5
    @test c3.total_bits == 5

    # Edge case: all zeros (degenerate)
    c4 = CompactHamilton{UInt64, UInt32}([0, 0])
    @test c4.mmax == 0
    @test c4.total_bits == 0

    # Error: empty vector
    @test_throws ArgumentError CompactHamilton{UInt64, UInt32}(Int[])

    # Error: negative values
    @test_throws ArgumentError CompactHamilton{UInt64, UInt32}([-1, 2])

    # Error: too many bits for index type
    @test_throws ArgumentError CompactHamilton{UInt8, UInt32}([5, 5])  # 10 bits > 8

    # Error: too many bits for coordinate type
    @test_throws ArgumentError CompactHamilton{UInt64, UInt8}([10])  # 10 > 8
end


@testitem "CompactHamilton convenience constructor type selection" begin
    using BijectiveHilbert: CompactHamilton, index_type, axis_type

    # sum(m)=9 fits in UInt8 (8 bits)? No -> UInt16; max(m)=4 fits in UInt8
    c1 = CompactHamilton([3, 2, 4])
    @test index_type(c1) == UInt16
    @test axis_type(c1) == UInt8

    # sum(m)=6 fits in UInt8; max(m)=3 fits in UInt8
    c2 = CompactHamilton([2, 2, 2])
    @test index_type(c2) == UInt8
    @test axis_type(c2) == UInt8

    # sum(m)=8 fits in UInt8; max(m)=8 fits in UInt8
    c3 = CompactHamilton([8])
    @test index_type(c3) == UInt8
    @test axis_type(c3) == UInt8

    # sum(m)=9 needs UInt16; max(m)=9 needs UInt16
    c4 = CompactHamilton([9])
    @test index_type(c4) == UInt16
    @test axis_type(c4) == UInt16

    # sum(m)=16 fits in UInt16; max(m)=8 fits in UInt8
    c5 = CompactHamilton([8, 8])
    @test index_type(c5) == UInt16
    @test axis_type(c5) == UInt8

    # sum(m)=17 needs UInt32; max(m)=9 needs UInt16
    c6 = CompactHamilton([9, 8])
    @test index_type(c6) == UInt32
    @test axis_type(c6) == UInt16

    # sum(m)=32 fits in UInt32; max(m)=16 fits in UInt16
    c7 = CompactHamilton([16, 16])
    @test index_type(c7) == UInt32
    @test axis_type(c7) == UInt16

    # sum(m)=33 needs UInt64; max(m)=17 needs UInt32
    c8 = CompactHamilton([17, 16])
    @test index_type(c8) == UInt64
    @test axis_type(c8) == UInt32

    # sum(m)=60 fits in UInt64; max(m)=20 fits in UInt32
    c9 = CompactHamilton([20, 20, 20])
    @test index_type(c9) == UInt64
    @test axis_type(c9) == UInt32

    # sum(m)=65 needs UInt128; max(m)=33 needs UInt64
    c10 = CompactHamilton([33, 32])
    @test index_type(c10) == UInt128
    @test axis_type(c10) == UInt64
end


@testitem "CompactHamilton basic encode/decode" begin
    using BijectiveHilbert: CompactHamilton, encode_hilbert_zero, decode_hilbert_zero!

    # Uniform case
    c = CompactHamilton{UInt64, UInt32}([3, 3, 3])
    X = UInt32[5, 2, 7]
    h = encode_hilbert_zero(c, X)
    Y = zeros(UInt32, 3)
    decode_hilbert_zero!(c, Y, h)
    @test X == Y

    # Anisotropic case
    c2 = CompactHamilton{UInt64, UInt32}([3, 2, 4])
    X2 = UInt32[5, 2, 11]
    h2 = encode_hilbert_zero(c2, X2)
    Y2 = zeros(UInt32, 3)
    decode_hilbert_zero!(c2, Y2, h2)
    @test X2 == Y2

    # Edge case: single dimension
    c3 = CompactHamilton{UInt64, UInt32}([4])
    X3 = UInt32[9]
    h3 = encode_hilbert_zero(c3, X3)
    Y3 = zeros(UInt32, 1)
    decode_hilbert_zero!(c3, Y3, h3)
    @test X3 == Y3

    # Edge case: all zeros input
    c4 = CompactHamilton{UInt64, UInt32}([2, 2])
    X4 = UInt32[0, 0]
    h4 = encode_hilbert_zero(c4, X4)
    Y4 = zeros(UInt32, 2)
    decode_hilbert_zero!(c4, Y4, h4)
    @test X4 == Y4
end


@testitem "CompactHamilton is its own inverse (uniform)" setup=[HilbertTestSuite] begin
    using BijectiveHilbert: CompactHamilton
    for b in 1:4, n in 2:4
        c = CompactHamilton{UInt64, UInt32}(fill(b, n))
        @test HilbertTestSuite.check_own_inverse(c, b, n)
    end
end


@testitem "CompactHamilton is its own inverse (anisotropic)" setup=[HilbertTestSuite] begin
    using BijectiveHilbert: CompactHamilton

    # Test various anisotropic configurations
    test_cases = [
        [3, 2, 4],
        [2, 3],
        [1, 2, 3],
        [4, 2, 3, 1],
        [2, 2, 3],
        [1, 1, 1, 1],
    ]

    for ms in test_cases
        c = CompactHamilton{UInt64, UInt32}(ms)
        @test HilbertTestSuite.check_own_inverse(c, ms)
    end
end


@testitem "CompactHamilton is complete set (uniform)" setup=[HilbertTestSuite] begin
    using BijectiveHilbert: CompactHamilton
    for n in 2:3, b in 2:3
        c = CompactHamilton{UInt64, UInt32}(fill(b, n))
        @test HilbertTestSuite.check_complete_set(c, b, n)
    end
end


@testitem "CompactHamilton type stability" begin
    using BijectiveHilbert
    using BijectiveHilbert: CompactHamilton, encode_hilbert_zero, decode_hilbert_zero!
    using BijectiveHilbert: affine_apply, affine_apply_inv, child_entry, child_dir, embed_state

    # Test helper functions
    @test @inferred(affine_apply(UInt32(5), UInt32(3), 2, 4)) isa UInt32
    @test @inferred(affine_apply_inv(UInt32(5), UInt32(3), 2, 4)) isa UInt32
    @test @inferred(child_entry(UInt32(5), 4)) isa UInt32
    @test @inferred(child_dir(5, 4)) isa Int
    @test @inferred(child_dir(UInt32(5), 4)) isa Int

    # Test embed_state with vectors
    A_old = [1, 3]
    pos_new = [2, -1, 1]
    e_old, d_old = UInt32(0b01), 0
    result = @inferred(embed_state(A_old, 2, pos_new, e_old, d_old))
    @test result isa Tuple{UInt32, Int}

    # Test embed_state with views (as used in actual code)
    axes_level = [1 1; 3 2; 0 3]
    pos_level = [2 1; -1 2; 1 3]
    A_view = @view axes_level[1:2, 1]
    pos_view = @view pos_level[:, 2]
    result2 = @inferred(embed_state(A_view, 2, pos_view, UInt32(1), 0))
    @test result2 isa Tuple{UInt32, Int}

    # Test top-level encode/decode
    c = CompactHamilton{UInt64, UInt32}([3, 2, 4])
    X = UInt32[5, 2, 11]
    @test @inferred(encode_hilbert_zero(c, X)) isa UInt64

    Y = zeros(UInt32, 3)
    h = UInt64(42)
    @inferred decode_hilbert_zero!(c, Y, h)
end


@testitem "CompactHamilton 1-based API" begin
    using BijectiveHilbert: CompactHamilton, encode_hilbert, decode_hilbert!

    c = CompactHamilton{UInt64, UInt32}([3, 2, 4])

    # 1-based coordinates and index
    X = UInt32[6, 3, 12]  # 1-based: corresponds to 0-based [5, 2, 11]
    h = encode_hilbert(c, X)
    @test h >= 1  # 1-based index

    Y = zeros(UInt32, 3)
    decode_hilbert!(c, Y, h)
    @test X == Y
end


@testitem "CompactHamilton with different type combinations" begin
    using BijectiveHilbert: CompactHamilton, encode_hilbert_zero, decode_hilbert_zero!

    # UInt128 index for large grids
    c1 = CompactHamilton{UInt128, UInt64}([20, 20, 20])
    X1 = UInt64[12345, 67890, 11111]
    h1 = encode_hilbert_zero(c1, X1)
    Y1 = zeros(UInt64, 3)
    decode_hilbert_zero!(c1, Y1, h1)
    @test X1 == Y1

    # UInt32 index, UInt16 coordinates
    c2 = CompactHamilton{UInt32, UInt16}([4, 4])
    X2 = UInt16[10, 5]
    h2 = encode_hilbert_zero(c2, X2)
    Y2 = zeros(UInt16, 2)
    decode_hilbert_zero!(c2, Y2, h2)
    @test X2 == Y2
end


@testitem "CompactHamilton encode/decode with mismatched coordinate types" begin
    using BijectiveHilbert: CompactHamilton, encode_hilbert_zero, decode_hilbert_zero!
    using BijectiveHilbert: encode_hilbert, decode_hilbert!

    # Create CompactHamilton with UInt64 index, UInt32 internal coordinate type
    c = CompactHamilton{UInt64, UInt32}([3, 2, 4])

    # Encode with Int64 coordinates (different from UInt32)
    X_int64 = Int64[5, 2, 11]
    h = encode_hilbert_zero(c, X_int64)

    # Decode into Int64 vector
    Y_int64 = zeros(Int64, 3)
    decode_hilbert_zero!(c, Y_int64, h)
    @test X_int64 == Y_int64

    # Encode with Int32 coordinates
    X_int32 = Int32[5, 2, 11]
    h2 = encode_hilbert_zero(c, X_int32)
    @test h == h2  # Same point should give same Hilbert index

    # Decode into UInt16 vector
    Y_uint16 = zeros(UInt16, 3)
    decode_hilbert_zero!(c, Y_uint16, h)
    @test Y_uint16 == UInt16[5, 2, 11]

    # Test 1-based API with mismatched types
    X_1based = Int64[6, 3, 12]
    h_1based = encode_hilbert(c, X_1based)
    Y_1based = zeros(Int32, 3)
    decode_hilbert!(c, Y_1based, h_1based)
    @test Y_1based == Int32[6, 3, 12]
end


@testitem "CompactHamilton constructor accepts different integer vector types" begin
    using BijectiveHilbert: CompactHamilton, encode_hilbert_zero, decode_hilbert_zero!
    using StaticArrays

    # Construct with Int32 vector
    c1 = CompactHamilton{UInt64, UInt32}(Int32[3, 2, 4])
    @test c1.mmax == 4

    # Construct with UInt8 vector
    c2 = CompactHamilton{UInt64, UInt32}(UInt8[3, 2, 4])
    @test c2.mmax == 4

    # Construct with tuple via SVector
    c3 = CompactHamilton{UInt64, UInt32}(SVector(3, 2, 4))
    @test c3.mmax == 4

    # All should produce equivalent encoders
    X = UInt32[5, 2, 11]
    @test encode_hilbert_zero(c1, X) == encode_hilbert_zero(c2, X) == encode_hilbert_zero(c3, X)
end


@testitem "CompactHamilton decode accepts Int index" begin
    using BijectiveHilbert

    # Test constructor with signed type
    c = CompactHamilton(Int64, [3, 2, 4])
    @test index_type(c) == UInt64

    # Test decode with Int literal
    X = zeros(Int, 3)
    decode_hilbert_zero!(c, X, 5)
    @test encode_hilbert_zero(c, X) == index_type(c)(5)
    decode_hilbert!(c, X, 6)
    @test all(x -> x >= 1, X)
end


@testitem "CompactHamilton encode accepts Int coordinates" begin
    using BijectiveHilbert

    c = CompactHamilton(Int64, [3, 2, 4])
    X_int = [5, 2, 11]
    X_uint = UInt8[5, 2, 11]
    @test encode_hilbert_zero(c, X_int) == encode_hilbert_zero(c, X_uint)
    @test encode_hilbert(c, X_int .+ 1) == encode_hilbert(c, X_uint .+ 1)
end
